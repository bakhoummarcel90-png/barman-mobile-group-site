...
export default function QuoteForm() {
  ...
  function validate(): boolean {
    ...
    if (!consent) next.nom = next.nom ?? "Merci d'autoriser Barman Mobile Group à vous recontacter.";
    setErrors(next);
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-10">
      ...
      {errors.nom?.includes("recontacter") && (
        <p className="text-sm text-terre-light">{errors.nom}</p>
      )}
